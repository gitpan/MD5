...
strings contained in the I<LIST> parameter.  The context is updated
after each element is added so that C<add('foo', 'bar')> may give
different results to C<add('foobar')>.
...
hexdecimal digits. This is exactly the same operation as performed by
the B<unpack> operation in the following example.
